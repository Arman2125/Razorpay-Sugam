...
Presentation only: templates use WhatsApp's own markdown (*bold*, single
asterisk — WhatsApp renders this as bold, not literal asterisks) plus emoji
for quick visual scanning. This is purely how an already-decided outcome is
worded — it changes no control flow, no data, and no outcome classification
...
def format_clarification(tool_name: str, candidates: list[dict]) -> str:
    ...
    if tool_name == "send_payment_reminder":
        ...
        for i, c in enumerate(candidates[:MAX_LIST_ITEMS], start=1):
            lines.append(
                f"{i}. {_fmt_amount(c.get('amount'))} — due *{_fmt_date(c.get('dueDate'))}* — {c.get('status')}"
            )
...
def format_known_error(code: str, message: str) -> str:
    friendly = {
        "DUPLICATE_REMINDER": "ℹ️ A reminder for this payment was already sent in the last 24 hours, so I didn't send another one.",
        "INVALID_TRANSITION": f"⚠️ I couldn't make that change: {message}",
        "INVALID_STATUS": f"⚠️ That's not a valid status: {message}",
        ...
        "NO_STATE_CHANGE": "ℹ️ That's already its current status, so there's nothing to change.",
        ...
        "PAYMENT_NOT_PAID": f"⚠️ That payment isn't in a paid state: {message}",
        ...
        "INVOICE_NOT_DRAFT": f"⚠️ I couldn't edit that invoice: {message}",
        ...
        "ALREADY_PAID": "ℹ️ That's already been paid, so there's nothing more to do.",
        "LINK_NOT_ACTIVE": f"⚠️ That payment link isn't active anymore: {message}",
...
def format_tool_success(tool_name: str, result: dict) -> str:
    ...
    if tool_name == "create_refund":
        r = result.get("refund", {})
        return f"↩️ Refunded {_fmt_amount(r.get('amount'))} for payment *{r.get('paymentId')}*."

    if tool_name == "get_refund":
        r = result.get("refund", {})
        return f"↩️ Refund *{r.get('refundId')}*: {_fmt_amount(r.get('amount'))} for payment *{r.get('paymentId')}*, {r.get('status')}."

    if tool_name in {"get_refunds", "get_payment_refunds"}:
        items = result.get("items", [])
        if not items:
            return "↩️ No matching refunds found."
        total = sum(r.get("amount", 0) for r in items)
        return f"↩️ Found *{len(items)}* refund(s) totaling {_fmt_amount(total)}."
    ...
    if tool_name == "process_due_subscriptions":
        ...
        return f"⚙️ Processed *{processed}* due subscription(s) — *{billed}* billed."
    ...
    if tool_name == "get_activity":
        items = result.get("items", [])
        if not items:
            return "🗒️ No matching activity found."
        return f"🗒️ Found *{len(items)}* activity record(s)."
